...
class DistanceMap:
    ...
    def get(self, agents) -> np.ndarray:
        """
        ...
        if self.reset_was_called:
            ...
            if compute_distance_map:
                self._compute(agents, self.rail)
    ...
    def get_shortest_paths(self, max_depth: Optional[int] = None, agents = [], agent_handle: Optional[int] = None) -> Dict[int, Optional[List[Waypoint]]]:
        """
        ...
        def _shortest_path_for_agent(agent):
            ...
            while (position != agent.target and (max_depth is None or depth < max_depth)):
                ...
                for next_action in next_actions:
                    next_action_distance = self.get(agents)[
